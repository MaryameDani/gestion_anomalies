...
const MaintenancierSuperviser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        const [ticketsResponse, vehiculesResponse, maintenanceUsersResponse] = await Promise.all([
  ...
  // Filter tickets to exclude "CLOTURE" status
  const filteredTickets = tickets.filter(
    (ticket) => ticket.statut !== "CLOTURE" && ticket.statut !== "ARCHIVE"
  )
        ...
        <Box sx={{ mb: 4, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            <Typography variant="h4" sx={{ fontWeight: "bold", color: "primary.main" }}>
              Suivi des Tickets Camions
            </Typography>
            <Typography variant="subtitle1" color="text.secondary" gutterBottom>
              Visualisez l'état des anomalies des camions
            </Typography>
          </Box>
          <Button
            variant="contained"
            ...
            <Grid container spacing={3}>
              {filteredTickets.length > 0 ? (
                      ...
                      >
                        <CardContent sx={{ flexGrow: 1 }}>
                          ...
                          <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
                            <PersonIcon sx={{ mr: 1, color: "#4A919E" }} />
                            <Typography variant="body2">
                              <strong>Assigné à maintenancier:</strong>{" "}
                              {ticket.utilisateur_assigne_maintenancier
                                ? getAssignedUserName(ticket.utilisateur_assigne_maintenancier)
                                : "Non assigné"}
                            </Typography>
                          </Box>
                ...
                <
                  ...
                  <Paper
                    elevation={2}
                    sx={{ p: 3, textAlign: "center", mt: 4, borderRadius: "8px" }}
                  >
